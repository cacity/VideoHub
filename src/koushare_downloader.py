...
def parse_koushare_url(url: str) -> tuple:
    ...
    if hostname not in {"koushare.com", "www.koushare.com"}:
        raise ValueError(f"不是有效的蔻享链接: {url}")
    # live 类型
...
def is_koushare_url(url: str) -> bool:
    """判断 URL 是否为可解析的蔻享视频链接。"""
    try:
        parse_koushare_url(url)
        return True
    ...
    try:
        ...
        if live_id:
            ...
            if video_item:
                ...
                if raw_title:
                    title = sanitize_filename(raw_title) or f"koushare_{video_id}"
