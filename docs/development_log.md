...
- 恢复蔻享专用下载器，支持回放链接与普通视频详情链接。
- 桌面端恢复直接粘贴识别、账号登录、手动 Token、即时下载和闲时队列处理。
- 本地 API 与 Chrome 扩展恢复蔻享任务创建和页面按钮注入。
- 下载文件统一保存至 `workspace/koushare/`；忽略规则补充旧版下载目录和 HLS 临时清单。

### 设计与实现

- 复用历史签名算法和蔻享播放接口，但按当前 `paths_config.py`、工作线程、闲时队列和扩展结构重新接入，避免覆盖后续平台功能。
- 登录密码不持久化；接口返回的 Access Token 仅保存到本机已忽略的 `.env`，源码和日志不记录 Token 内容。
- 下载器根据 FHD、HD、SD 顺序选择可用 HLS 地址，并通过 FFmpeg 合并为 MP4。
- 对链接域名和页面路径做严格校验，文件名执行 Windows 非法字符替换和长度限制。

### 验证结果

- 新增 11 项单元测试，覆盖链接解析、签名、Token 设置与清除、画质回退和完整下载编排。
- 全部测试 `76 passed`；Python 语法检查、扩展 Manifest JSON 和三份 JavaScript 语法检查通过。
- 使用公开蔻享回放链接完成不落盘接口探测，当前接口可返回 FHD、HD、SD 三档播放信息。
- `git diff --check` 通过，README 和宣传页面未增加蔻享介绍。
